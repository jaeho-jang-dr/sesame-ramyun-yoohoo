...
## 📌 기본 원칙 (Core Principle)
> [!IMPORTANT]
> 앞으로 이 프로젝트의 모든 앱 코딩 과정은 아래 규칙을 **상시적이고 절대적인 기본 워크플로우**로 삼아 진행합니다.
> 1. **Gemini**: 요구사항 분석, 설계, 기능 계획 수립 및 지침 파일(`CLAUDE_TASKS.md`) 작성.
> 2. **Claude CLI**: 제공된 오더를 바탕으로 세부 소스 코드 구현, 디버깅 및 유닛 테스트 진행 후 완료 보고.
> 3. **Gemini**: 완료된 코드를 전달받아 로컬 빌드 및 종합 테스트를 수행하고, 이상 없을 시 Git 스테이징/커밋(메모리 업데이트) 및 원격 저장소 푸시(배포) 진행.
