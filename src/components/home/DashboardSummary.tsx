    ...
    return (
        <section className="cartoon-card bg-white p-6">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs bg-gray-900 text-white px-2.5 py-1 rounded-full font-bold">Today</span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {stats.map((stat, i) => (
                    <div key={i} className={`${stat.colorClass} p-4 rounded-2xl border-4 border-gray-900 shadow-cartoon-sm flex flex-col items-center justify-center text-center gap-1 transition-transform duration-200 hover:-translate-y-1 hover:scale-105`}>
